...
public abstract class AbstractRouter extends AbstractSingleInMultiOutPipeComponent implements Router {
	 ...
	 * {@link NullEngineTick} allows super classes to generate new events if they
	 * can.
	 */
	@Override
	 ...
	 * {@link Element}s as long as there is a downstream component that is available
	...
	public boolean canReceiveElement(ElementTransferData pipeData) {
		...
		// available to receive. On the first available, break out of the for loop, no
	 ...
	 * {@link Router}; {@link #canReceiveElement(ElementTransferData)} is true if
	 * there exists a downstream {@link Component} available to receive.
	 */
	@Override
	...
	protected void generateEvents(EngineTick tick) {
		if (pipeData != null && transferOut != null) {
			...
			// After we've used the data, and the TransferOutputPipe, set them to null, so
